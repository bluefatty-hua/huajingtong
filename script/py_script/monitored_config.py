...
MONITOR_DIC = {
    'rpt_day_all': {
        ...
        'judge_sql': '''
                     ...
                     FROM bireport.rpt_day_all n1
                     INNER JOIN (SELECT * FROM stage.monitored WHERE table_name = 'rpt_day_all') m ON n1.dt = m.dt AND n1.platform = m.platform
                     LEFT JOIN (SELECT * 
    ...
    ''',
        'insert_sql': '''
            ...
            SELECT dt, platform, anchor_cnt, live_cnt, revenue, 'rpt_day_all' AS table_name
            FROM bireport.rpt_day_all
            WHERE newold_state = 'all'
              AND active_state = 'all'
              AND revenue_level = 'all'
