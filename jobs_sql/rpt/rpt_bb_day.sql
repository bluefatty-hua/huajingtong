...
FROM (SELECT dt,
             ...
             MAX(backend_account_id)      AS backend_account_id,
             IFNULL(remark, 'all')        AS remark,
      ...
      WHERE (backend_account_id != 'all' OR revenue_level != 'all' OR newold_state != 'all' OR active_state != 'all')
        AND dt BETWEEN '{start_date}' AND '{end_date}'
      GROUP BY dt, remark, revenue_level, newold_state, active_state
      WITH ROLLUP
      ...
      WHERE (backend_account_id != 'all' OR revenue_level != 'all' OR newold_state != 'all' OR active_state != 'all')
        AND dt BETWEEN '{start_date}' AND '{end_date}'
      GROUP BY dt, revenue_level, newold_state, active_state, remark
      WITH ROLLUP
      ...
      WHERE (backend_account_id != 'all' OR revenue_level != 'all' OR newold_state != 'all' OR active_state != 'all')
        AND dt BETWEEN '{start_date}' AND '{end_date}'
      GROUP BY dt, newold_state, active_state, remark, revenue_level
      WITH ROLLUP
      ...
      WHERE (backend_account_id != 'all' OR revenue_level != 'all' OR newold_state != 'all' OR active_state != 'all')
        AND dt BETWEEN '{start_date}' AND '{end_date}'
      GROUP BY dt, active_state, remark, revenue_level, newold_state
      WITH ROLLUP
    ...
    SELECT dt,
      ...
      WHERE (backend_account_id != 'all' OR revenue_level != 'all' OR newold_state != 'all' OR active_state != 'all')
        AND dt BETWEEN '{start_date}' AND '{end_date}'
      GROUP BY dt, active_state, revenue_level, remark, newold_state
      WITH ROLLUP
     ) t
...
select * from
(SELECT dt,remark,revenue_level,newold_state,active_state,'主播数' as idx,
anchor_cnt as val FROM `bireport`.`rpt_day_bb_guild_new` where dt BETWEEN '{start_date}' AND '{end_date}' and  revenue_level!='all'
union 
SELECT dt,remark,revenue_level,newold_state,active_state,'开播数' as idx,
live_cnt as val FROM `bireport`.`rpt_day_bb_guild_new` where dt BETWEEN '{start_date}' AND '{end_date}' and revenue_level!='all'
union 
SELECT dt,remark,revenue_level,newold_state,active_state,'流水' as idx,
revenue as val FROM `bireport`.`rpt_day_bb_guild_new` where dt BETWEEN '{start_date}' AND '{end_date}' and revenue_level!='all'
union 
SELECT dt,remark,revenue_level,newold_state,active_state,'开播数' as idx,
round(revenue/live_cnt,0) as val FROM `bireport`.`rpt_day_bb_guild_new` where dt BETWEEN '{start_date}' AND '{end_date}' and  revenue_level!='all' and live_cnt>0) t
