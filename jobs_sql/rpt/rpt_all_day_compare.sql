...
INSERT INTO bireport.rpt_day_all_compare
(dt,idx,yy_val)
SELECT dt,'主播数',anchor_cnt FROM  bireport.rpt_day_all
where dt  >='{month}' and dt <= LAST_DAY('{month}') and
...
INSERT INTO bireport.rpt_day_all_compare
(dt,idx,yy_val)
SELECT dt,'开播数',live_cnt FROM  bireport.rpt_day_all
where dt  >='{month}' and dt <= LAST_DAY('{month}') and
platform = 'YY' and revenue_level = 'all' and newold_state = 'all' and active_state = 'all'
ON DUPLICATE KEY UPDATE yy_val=values(yy_val);
...
INSERT INTO bireport.rpt_day_all_compare
(dt,idx,yy_val)
SELECT dt,'流水',revenue FROM  bireport.rpt_day_all
where dt  >='{month}' and dt <= LAST_DAY('{month}') and
platform = 'YY' and revenue_level = 'all' and newold_state = 'all' and active_state = 'all'
ON DUPLICATE KEY UPDATE yy_val=values(yy_val);
...
INSERT INTO bireport.rpt_day_all_compare
(dt,idx,yy_val)
SELECT dt,'开播人均流水',if(live_cnt>0,revenue/live_cnt,0) FROM  bireport.rpt_day_all
where dt  >='{month}' and dt <= LAST_DAY('{month}') and
platform = 'YY' and revenue_level = 'all' and newold_state = 'all' and active_state = 'all'
ON DUPLICATE KEY UPDATE yy_val=values(yy_val);
...
INSERT INTO bireport.rpt_day_all_compare
(dt,idx,yy_val)
SELECT dt,'时长',duration FROM  bireport.rpt_day_all
where dt  >='{month}' and dt <= LAST_DAY('{month}') and
platform = 'YY' and revenue_level = 'all' and newold_state = 'all' and active_state = 'all'
ON DUPLICATE KEY UPDATE yy_val=values(yy_val);
...
INSERT INTO bireport.rpt_day_all_compare
(dt,idx,yy_val)
SELECT dt,'开播率',if(anchor_cnt>0,round(live_cnt*100/anchor_cnt,0),0) FROM  bireport.rpt_day_all
where dt  >='{month}' and dt <= LAST_DAY('{month}') and
platform = 'YY' and revenue_level = 'all' and newold_state = 'all' and active_state = 'all'
ON DUPLICATE KEY UPDATE yy_val=values(yy_val);
...
INSERT INTO bireport.rpt_day_all_compare
(dt,idx,yy_val)
SELECT dt,'开播人均时长',if(live_cnt>0,round(duration/live_cnt,0),0) FROM  bireport.rpt_day_all
where dt  >='{month}' and dt <= LAST_DAY('{month}') and
platform = 'YY' and revenue_level = 'all' and newold_state = 'all' and active_state = 'all'
ON DUPLICATE KEY UPDATE yy_val=values(yy_val);
