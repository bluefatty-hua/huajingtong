...
DELETE
FROM stage.stage_rpt_fx_month_anchor_live_contrast
WHERE dt = '{month}';
INSERT IGNORE INTO stage.stage_rpt_fx_month_anchor_live_contrast
SELECT dt,
       platform_name,
       platform_id,
       anchor_no,
...
-- CREATE TABLE stage.stage_rpt_fx_month_anchor_add_loss AS
DELETE
FROM stage.stage_rpt_fx_month_anchor_add_loss
WHERE add_loss_state = 'add'
  AND dt = '{month}';
INSERT INTO stage.stage_rpt_fx_month_anchor_add_loss
SELECT al1.dt, al1.platform_name, al1.platform_id, al1.anchor_no, 'add' AS add_loss_state
FROM stage.stage_rpt_fx_month_anchor_live_contrast al1
         LEFT JOIN stage.stage_rpt_fx_month_anchor_live_contrast al2
...
# CREATE TABLE stage.stage_rpt_fx_month_guild_live
DELETE
FROM stage.stage_rpt_fx_month_guild_live
WHERE dt = '{month}';
INSERT INTO stage.stage_rpt_fx_month_guild_live
SELECT DATE_FORMAT(al.dt, '%Y-%m-01')                                                        AS dt,
       ...
       SUM(IF(add_loss_state <> 'loss' AND al.anchor_income > 0, al.anchor_income, 0)) / 125 AS anchor_income,
       SUM(IF(add_loss_state <> 'loss' AND al.revenue_orig > 0, al.revenue_orig, 0))         AS anchor_income_orig,
       SUM(IF(add_loss_state <> 'loss' AND al.guild_income > 0, al.guild_income, 0)) / 125   AS guild_income,
...
FROM (
         ...
         FROM warehouse.dw_fx_month_anchor_live al
                  LEFT JOIN stage.stage_rpt_fx_month_anchor_add_loss aal
         ...
         FROM warehouse.dw_fx_month_anchor_live al
                  INNER JOIN stage.stage_rpt_fx_month_anchor_add_loss aal
...
SELECT gl.dt,
       ...
       gl.guild_income / 125  AS guild_income,
       gl.anchor_income       AS guild_income_orig,
       gl.anchor_income / 125 AS anchor_income,
       gl.anchor_income       AS anchor_income_orig
# FROM warehouse.dw_fx_month_guild_live gl
FROM stage.stage_rpt_fx_month_guild_live gl
WHERE gl.dt = '{month}'
;


REPLACE INTO bireport.rpt_month_fx_guild (dt, platform_id, platform, backend_account_id, revenue_level,
...
SELECT t1.dt,
       ...
       0                                                               AS `guild_income`,
       0                                                               AS `anchor_income`
FROM bireport.rpt_month_fx_guild t1
         LEFT JOIN bireport.rpt_month_fx_guild t3
