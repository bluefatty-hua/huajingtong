...
anchor_cnt as val FROM `bireport`.`rpt_month_yy_guild_new` where dt = '{month}' and  revenue_level!='all'
union 
SELECT dt,channel_num,revenue_level,newold_state,active_state,'开播数' as idx,
live_cnt as val FROM `bireport`.`rpt_month_yy_guild_new` where dt = '{month}' and revenue_level!='all'
union 
SELECT dt,channel_num,revenue_level,newold_state,active_state,'流水' as idx,
revenue as val FROM `bireport`.`rpt_month_yy_guild_new` where dt = '{month}' and revenue_level!='all'
union 
SELECT dt,channel_num,revenue_level,newold_state,active_state,'开播人均流水' as idx,
round(revenue/live_cnt,0) as val FROM `bireport`.`rpt_month_yy_guild_new` where dt = '{month}' and  revenue_level!='all' and live_cnt>0) t