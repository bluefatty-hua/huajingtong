...
SELECT t1.dt,
       ...
       t1.newold_state,
       t1.active_state,
       if(t1.new_anchor_cnt=0,null,concat(ROUND(t1.new_r30_cnt/t1.new_anchor_cnt*100,1),'%'))  as new_r30_ratio
FROM bireport.rpt_day_bb_guild t1
WHERE t1.dt >= '{month}'
  AND t1.dt <= LAST_DAY('{month}')
ON DUPLICATE KEY UPDATE `new_r30_ratio`=values(new_r30_ratio);
