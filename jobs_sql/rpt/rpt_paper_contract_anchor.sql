...
SELECT '{month}'                                                                 AS dt,
       al.platform_name,
       al.id_card,
       IF((al.anchor_no = '' OR al.anchor_no rlike '无'), '未知', al.anchor_no)     AS anchor_no,
       al.real_name,
       ...
       IFNULL(al3.dt, '{month}' - INTERVAL 1 MONTH)                              AS dt_t1,
       IFNULL(al3.live_days, 0)                                                  AS live_days_t1,
       ROUND(IFNULL(al3.revenue_rmb, 0), 0)                                      AS revenue_rmb_t1,
       DATEDIFF(LAST_DAY(al3.dt), al3.dt) + 1 - al3.live_days                    AS unlive_days_t1,

       IFNULL(al4.dt, '{month}')                                                 AS dt_t,
       IFNULL(al4.live_days, 0)                                                  AS live_days_t,
       ROUND(IFNULL(al4.revenue_rmb, 0), 0)                                      AS revenue_rmb_t,
       DATEDIFF(CASE
