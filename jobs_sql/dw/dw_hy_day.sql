...
DELETE FROM  stage.stage_huya_day_anchor_info  WHERE dt BETWEEN '{start_date}' AND '{end_date}';
...
INSERT IGNORE INTO stage.stage_huya_day_anchor_info
(anchor_uid,channel_id, `comment` ,dt)
SELECT anchor_uid,channel_id,'from anchor_live_detail_day', dt 
...
LEFT JOIN warehouse.ods_huya_day_guild_live_income_gift cgi ON cd.dt = cgi.dt AND cd.channel_id = cgi.channel_id
LEFT JOIN warehouse.ods_huya_day_guild_live_income_guard cgu ON cd.dt = cgu.dt AND cd.channel_id = cgu.channel_id
