...
delete from stage.stage_huya_month_guild_info where dt >='{month.start}' AND dt<='{month.end}';
...
delete from stage.stage_huya_month_anchor_info where dt >='{month.start}' AND dt<='{month.end}';
...
FROM (select anchor_uid,channel_id,
  ...
  from `warehouse`.`dw_huya_day_anchor_live` where dt BETWEEN '{month.start}' AND '{month.end}' GROUP BY anchor_uid,channel_id) t1
...
delete from stage.stage_huya_month_guild_live_revenue where dt ='{month.start}';
...
delete from stage.stage_huya_month_guild_live_gift_income where dt ='{month.start}';
...
delete from stage.stage_huya_month_guild_live_guard_income where dt ='{month.start}';
...
delete from stage.stage_huya_month_guild_live_noble_income where dt ='{month.start}';
...
SELECT
  ...
  guard_income,
  noble_income
FROM (SELECT channel_id FROM `warehouse`.`dw_huya_day_guild_live` WHERE dt BETWEEN '{month.start}' AND '{month.end}' GROUP BY channel_id) t1
