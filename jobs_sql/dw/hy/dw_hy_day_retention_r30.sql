...
-- 30天留存
create table if not exists stage.stage_huya_30days_live
(
 dt date,
 anchor_uid bigint,
 live_days int,
...
delete from stage.stage_huya_30days_live 
where dt  >='{month}' and dt <= LAST_DAY('{month}'); 


-- 计算30天开播天数
insert into stage.stage_huya_30days_live 
SELECT
t1.dt,t1.anchor_uid,
...
create table if not exists stage.stage_huya_30th_missing
(
 dt date,
 anchor_uid bigint,
 PRIMARY KEY (`dt`,`anchor_uid`)
...
delete from stage.stage_huya_30th_missing 
where dt  >='{month}' and dt <= LAST_DAY('{month}'); 

-- 计算第30天不在主播列表主播
insert into stage.stage_huya_30th_missing 
SELECT
t1.dt,t1.anchor_uid
FROM warehouse.dw_huya_day_anchor_live t1 
...
-- 更新开播数据到dw
UPDATE warehouse.dw_huya_day_anchor_live t1,stage.stage_huya_30days_live t2
...
-- 更新流失数据到dw
UPDATE warehouse.dw_huya_day_anchor_live t1,stage.stage_huya_30th_missing t2
