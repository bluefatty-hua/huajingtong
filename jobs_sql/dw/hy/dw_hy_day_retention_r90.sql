...
-- 90天留存
create table if not exists stage.stage_huya_90days_live
(
 dt date,
 anchor_uid bigint,
 live_days int,
...
delete from stage.stage_huya_90days_live 
where dt  >='{month}' and dt <= LAST_DAY('{month}'); 

insert into stage.stage_huya_90days_live 
SELECT
t1.dt,t1.anchor_uid,
...
JOIN warehouse.dw_huya_day_anchor_live t2
	...
	AND t2.dt <= t1.dt + INTERVAL 90 DAY
...
create table if not exists stage.stage_huya_90th_missing
(
 dt date,
 anchor_uid bigint,
 PRIMARY KEY (`dt`,`anchor_uid`)
...
delete from stage.stage_huya_90th_missing 
where dt  >='{month}' and dt <= LAST_DAY('{month}');

insert into stage.stage_huya_90th_missing 
SELECT
t1.dt,t1.anchor_uid
FROM warehouse.dw_huya_day_anchor_live t1 
...
SET retention_r90_lives = 0,retention_r90_missing = 0,retention_r90=1
where dt  >='{month}' and dt <= LAST_DAY('{month}');


-- 更新开播数据到dw
UPDATE warehouse.dw_huya_day_anchor_live t1,stage.stage_huya_90days_live t2
SET t1.retention_r90_lives = t2.live_days
WHERE t1.dt  >='{month}' and t1.dt <= LAST_DAY('{month}') AND t1.add_loss_state='new' 
and t1.dt = t2.dt and t1.anchor_uid = t2.anchor_uid;


-- 更新流失数据到dw
UPDATE warehouse.dw_huya_day_anchor_live t1,stage.stage_huya_90th_missing t2
SET t1.retention_r90_missing = 1
WHERE t1.dt  >='{month}' and t1.dt <= LAST_DAY('{month}') AND t1.add_loss_state='new' 
and t1.dt = t2.dt and t1.anchor_uid = t2.anchor_uid;
