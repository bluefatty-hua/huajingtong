...
-- 30天留存
create table if not exists stage.stage_fx_30days_live
(
 dt date,
 anchor_no bigint,
 live_days int,
...
delete from stage.stage_fx_30days_live 
where dt  >='{month}' and dt <= LAST_DAY('{month}'); 


-- 计算30天开播天数
insert into stage.stage_fx_30days_live 
SELECT
t1.dt,t1.anchor_no,
...
create table if not exists stage.stage_fx_30th_missing
(
 dt date,
 anchor_no bigint,
 PRIMARY KEY (`dt`,`anchor_no`)
...
delete from stage.stage_fx_30th_missing 
where dt  >='{month}' and dt <= LAST_DAY('{month}'); 

-- 计算第30天不在主播列表主播
insert into stage.stage_fx_30th_missing 
SELECT
t1.dt,t1.anchor_no
FROM warehouse.dw_fx_day_anchor_live t1 
...
-- 更新开播数据到dw
UPDATE warehouse.dw_fx_day_anchor_live t1,stage.stage_fx_30days_live t2
...
-- 更新流失数据到dw
UPDATE warehouse.dw_fx_day_anchor_live t1,stage.stage_fx_30th_missing t2
