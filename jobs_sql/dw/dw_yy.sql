...
DELETE FROM warehouse.dw_month_yy_anchor WHERE dt = CONCAT(YEAR('{start_date}'), '-', MONTH('{start_date}'), '-01');
...
--        COUNT(DISTINCT CASE WHEN t.live_status = 1 THEN t.anchor_no ELSE NULL END)  AS live_anchor_cnt,
