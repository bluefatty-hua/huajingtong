...
WHERE DATE_FORMAT(date, '%Y-%m-%d') BETWEEN '2020-01-01' AND '2020-03-09'
;
...
SELECT ai.platform_id,
       ...
       CASE WHEN ain.live_time >= 0 AND ain.live_time <= 24 THEN ain.live_time ELSE 0 END           AS duration_hour,
       CASE WHEN ain.live_time >= 0 AND ain.live_time <= 24 THEN ain.live_time * 60 * 60 ELSE 0 END AS duration,
